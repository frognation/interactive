function play() {
  var audio = new Audio('https://cdn.glitch.global/89108c54-d2a1-4595-a136-f18e00b1bc44/1second.wav?v=1651691516651');
  audio.play();
}
...
function consoleLogRows() {
  ...
  rows.forEach((row) => {
    console.log("Row:", row.fields.answers);
  });
}

// 여기부터 텍스트 넣기
setTimeout(() => {
  // var last_text = rows.slice(-1).slice(-1)[0].fields.answers;
  var last_row = rows[rows.length - 1];
  var last_text = last_row.fields.answers;
  console.log("last text =" + last_text);
  $('#text-here').html(last_text);
}, 500);
